# LWM Simulator Training

This document preserves the detailed language-world-model simulator training plan. It is intentionally separate from the main OCR-to-EVMBench pipeline because it is longer, more mathematical, and future-facing.

## Current Objects And Artifacts

- No LWM transition dataset, simulator model, exact replay trace generator, or LWM-only eval harness is current in this docs directory.
- Current usable inputs for future LWM planning are source PDFs, OCR artifacts, planned EVMBench task provenance, and the post-training source inventory in `5_POST_TRAINING_DATASETS_AND_RECIPES.md`.

## Future LWM Work

- Build exact executor traces before training a simulator model.
- Train the LWM to predict state transitions and verifier outputs, not to replace exact EVM replay.
- Evaluate LWM outputs with simulator-fidelity metrics and keep exact replay as the proof path.
...
to complement `5_POST_TRAINING_DATASETS_AND_RECIPES.md`, which covers detect-mode SFT, DPO,
...
- `5_POST_TRAINING_DATASETS_AND_RECIPES.md` for existing audit-agent SFT, DPO, GRPO, and rollout record shapes.
- `5_POST_TRAINING_DATASETS_AND_RECIPES.md` for source pools and leakage boundaries.
